...
**Node.js compatibility** (`node/node/`) — ~12.6K LOC JS. Shims for `node:fs`, `node:net`, `node:dgram`, `node:http`, `node:child_process`, `node:stream`, `node:crypto`, `node:path`, `node:events`, `node:url`, `node:os`, `node:buffer`, `node:assert`, `node:test`, `node:sqlite`, `node:module`, `node:tty`, `node:readline` (line mode only), etc.
...
**qn modules** (`node/qn/`) — ~0.5K LOC JS + crypto native module. Higher-level APIs: crypto primitives (`qn:crypto`, BearSSL-backed), async TLS I/O (`qn:tls`, with optional certificate / SPKI pinning), HTTP server (`qn:http`), HTTPS fetch pin registry (`qn:fetch`), reverse proxy (`qn:proxy`), pseudo-terminal (`qn:pty`), TypeScript transform (`qn:sucrase`), package installer (`qn install`, with native support for a `sourceDependencies` field — Cargo-style declarative git deps with optional `exports` rewrite and `build` escape hatch), in-process git client (`qn:git` — local repos + smart-HTTP fetch with SHA-1 verification, used by `qn install` so the `git` binary is only needed for `git+ssh://`), bundler (`qn build` / `qn:bundle`), watch-mode runner (`qn --watch` / `qn:watch`, reuses `traceModuleGraph` from `qn:bundle`), libuv JS wrappers.
