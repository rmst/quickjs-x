...
| `structuredClone` | ⚠️ | No transferables (ArrayBuffer ownership transfer) |
...
| `isIP` / `isIPv4` / `isIPv6` | ❌ | |
| Unix domain sockets | ✅ | `connect(path)`, `createConnection(path)`, `server.listen(path)`, `socketPath`; Linux abstract namespace paths supported |
